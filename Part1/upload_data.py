...
def infer_table_schema(csv_file, table_name):
    """Infer table schema from its CSV file"""
    ...
    for header, value in zip(headers, row):
        if value == "":
            ...
            try:
                ...
                cleaned_row.append(None)  # Use None if conversion fails
        elif header == 'DAMAGE':
            try:
            ...
            # assuming other columns as strings
...
def load_data_into_table(connection, table_name, csv_file, batch_size=5000):
    ...
    try:
        ...
        with open(csv_file, 'r', encoding='utf-8') as file:
            ...
            # if any remaining rows after batches
            if data:
                cursor.executemany(insert_statement, data)
                connection.commit()
