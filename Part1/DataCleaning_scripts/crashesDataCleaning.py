"""
Unified progress tracking for processing and cleaning Crashes data with debug statements
"""

from tqdm import tqdm
from utils_crashes import DataProcessor, SpatialOperator
...
def crashes_data_cleaning(input_path, police_beat_path, vehicles_path, output_path):
    ...
    with tqdm(total=len(missing_latlng), desc="Step 2: Updating Lat/Lng/Street Names") as step2_pbar:
        for row in missing_latlng:
            lat, lon, loc = spatial.get_lat_long_location(row['STREET_NO'], row['STREET_NAME'])
            if lat and lon:
                row['LATITUDE'] = lat
                row['LONGITUDE'] = lon
                row['LOCATION'] = loc
    ...
    out_of_bounds_data = spatial.get_lat_lon_outside_chicago(data)
    data = spatial.correct_out_chicago(out_of_bounds_data, data)
    save_progress(data, fieldnames, output_path)

    # Step 4: Fill Missing Street Names
    spatial.process_data_street(data)
    save_progress(data, fieldnames, output_path)
    ...
    with tqdm(total=len(data), desc="Step 6: Adding H3 Encoding") as step6_pbar:
        spatial.add_h3_encoding(data, resolution=10)
        step6_pbar.update(len(data))
    save_progress(data, fieldnames, output_path)

    # Step 7: Load Police Beats and Perform Spatial Join
    police_beats = spatial.load_police_beats(police_beat_path)
    data = spatial.spatial_join(data, police_beats)
    save_progress(data, fieldnames, output_path)
